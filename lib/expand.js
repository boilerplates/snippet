...
function parse(str) {
  ...
  tabs.forEach(function (stop, i) {
    expanded = expanded.replace(stop, '__param' + (i++) + '__');
  });
  return expanded;
}
...
function interpolate(str) {
  return function (data) {
    return str.replace(/__param(\d+)__/g, function (m, num) {
      var key = 'param' + String(num);
      // var val = /param/.test(data[key]) ? data[key] : key;
...
// var snippet = render('ul>li.item$*5', ['a', 'b', 'c']);
...
// console.log(snippet)
// console.log(toSnippet('div>button{inner $}'))
